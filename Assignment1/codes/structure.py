...
def create_query(tokens,database):
	...
	inside_punctuation = filter_tokens(function_token[1].tokens);
	'''try:
		inside_punctuation=inside_punctuation.get_identifiers();
	except:
		inside_punctuation=[inside_punctuation];'''
	inside_punctuation=inside_punctuation[1:-1];
	inside_punctuation=filter_tokens_1(inside_punctuation);
	print inside_punctuation;

	attr=[];
	attr_type=[];
	i=0;
	while(i<len(inside_punctuation)):
		attr.append(inside_punctuation[i].value);
		attr_type.append(inside_punctuation[i+1].value);
		i+=2;
	#print attr;
	#print attr_type
	#for i in inside_punctuation:
		#attr_tokens=i;
		#attr.append(attr_tokens[0]);
		#attr_type.append(attr_tokens[1]);
